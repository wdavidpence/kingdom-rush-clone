#!/usr/bin/env python3
"""PenceMOA qwen2735 1.3.5 locked titan identity. Parallel, 3600s/request."""
import json, time, urllib.request, concurrent.futures
from pathlib import Path

LOG = Path("/Users/davidpence/kingdom-rush-clone/sprint-logs")
TASK = (LOG / "pass-1.3.5-moa-prompt.md").read_text()
...
CODE_SYS = (
    "Output unified diffs only. Target live const drawTitan = (ctx, frame = 0) => { "
    "and keep make(\"enemy_titan\", 80, 72, (ctx) => drawTitan(ctx, 0)); "
    "Do not change 88, 80. Do not edit drawScout, drawBrute, or drawFlyer. "
    ...
    "Optional game-data.js: only titan size 24 to 26. "
    "Do not touch { x: 100, y: 375 } or bannerY=98."
)
SYNTH_SYS = (
    "Synthesize ONE final set of diffs that apply to live drawTitan and make(\"enemy_titan\", 80, 72. "
    "Keep 88, 80. Do not touch drawScout, drawBrute, or drawFlyer. No fake APIs. No CONFIG. Optional: titan size 24 to 26 only."
)
...
def main():
    ...
    (LOG / "pass-1.3.5-moa-a.json").write_text(json.dumps(ca, indent=2)[:200000])
    (LOG / "pass-1.3.5-moa-b.json").write_text(json.dumps(cb, indent=2)[:200000])
    (LOG / "pass-1.3.5-moa-synth-input.txt").write_text(
        "A\n" + (ca["code"].get("text") or "") + "\nB\n" + (cb["code"].get("text") or "")
    )
    ...
    (LOG / "pass-1.3.5-moa-synth.txt").write_text(synth.get("text") or "")
    print(
        json.dumps(
            {
