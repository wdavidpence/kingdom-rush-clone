#!/usr/bin/env python3
"""PenceMOA qwen2735 1.2.0 Forest Gate set-piece. SAME assignment, parallel, long budget."""
import json, time, urllib.request, concurrent.futures
from pathlib import Path

LOG = Path("/Users/davidpence/kingdom-rush-clone/sprint-logs")
TASK = (LOG / "pass-1.2.0-moa-prompt.md").read_text()
...
CODE_SYS = (
    "Output unified diffs only. Prefer ```diff:public/src/krc-art.js then optional "
    "```diff:public/src/game.js. Rebuild gate_arch. Do not dump the whole atlas. "
    "Do not touch campaign nodes or Forest Gate { x: 100, y: 375 }."
)
SYNTH_SYS = (
    "Synthesize ONE final set of diffs. Output only ```diff fences for krc-art.js "
    "and optionally game.js. Keep a readable dark arch, lanterns, ivy, timber doors. "
    "Path ruts only on Forest Gate. No campaign node edits."
)
...
def main():
    ...
    (LOG / "pass-1.2.0-moa-a.json").write_text(json.dumps(ca, indent=2)[:200000])
    (LOG / "pass-1.2.0-moa-b.json").write_text(json.dumps(cb, indent=2)[:200000])
    (LOG / "pass-1.2.0-moa-synth-input.txt").write_text(
        "A\n" + (ca["code"].get("text") or "") + "\nB\n" + (cb["code"].get("text") or "")
    )
    ...
    (LOG / "pass-1.2.0-moa-synth.txt").write_text(synth.get("text") or "")
    print(
        json.dumps(
            {
