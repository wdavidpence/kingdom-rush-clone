#!/usr/bin/env python3
"""Single-worker qwen3.6-35b-ud mage crystal. Not MoA. 3600s/request."""
import json, time, urllib.request
from pathlib import Path

LOG = Path("/Users/davidpence/kingdom-rush-clone/sprint-logs")
TASK = (LOG / "pass-1.3.21-qwen35-prompt.md").read_text()
...
CODE_SYS = (
    "Output unified diffs only. Target live const drawMageCrystal128 = (ctx, isFire = false) => { "
...
def main():
    ...
    (LOG / "pass-1.3.21-qwen35.json").write_text(json.dumps(rec, indent=2)[:200000])
    (LOG / "pass-1.3.21-qwen35-code.txt").write_text(code.get("text") or "")
    print(
        json.dumps(
            {
                "ok": code.get("ok"),
