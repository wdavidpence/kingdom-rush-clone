...
CODE_SYS = (
    ...
    "Do not change 36, 36. Use only poly/linGrad/radGrad/ellipse/rounded/shadow/speckles "
    ...
    "Optional game.js edit is only the live setScale ternary. "
    "Do not touch { x: 100, y: 375 } or bannerY=98."
)
SYNTH_SYS = (
    "Synthesize ONE final set of diffs that apply to live make(\"projectile_magic\", 36, 36, (ctx) => {. "
    "Keep 36, 36. No fake APIs. No CONFIG. Optional game.js: only the live setScale line."
)
...
def main():
    ...
    (LOG / "pass-1.3.1b-moa-a.json").write_text(json.dumps(ca, indent=2)[:200000])
    (LOG / "pass-1.3.1b-moa-b.json").write_text(json.dumps(cb, indent=2)[:200000])
    (LOG / "pass-1.3.1b-moa-synth-input.txt").write_text(
        "A\n" + (ca["code"].get("text") or "") + "\nB\n" + (cb["code"].get("text") or "")
    )
    ...
    (LOG / "pass-1.3.1b-moa-synth.txt").write_text(synth.get("text") or "")
    print(
        json.dumps(
            {
