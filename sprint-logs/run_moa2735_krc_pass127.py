#!/usr/bin/env python3
"""PenceMOA qwen2735 1.2.7 Forest Gate. SAME assignment, parallel, long budget."""
import json, time, urllib.request, concurrent.futures
from pathlib import Path

LOG = Path("/Users/davidpence/kingdom-rush-clone/sprint-logs")
TASK = (LOG / "pass-1.2.7-moa-prompt.md").read_text()
...
def main():
    ...
    (LOG / "pass-1.2.7-moa-a.json").write_text(json.dumps(ca, indent=2)[:200000])
    (LOG / "pass-1.2.7-moa-b.json").write_text(json.dumps(cb, indent=2)[:200000])
    (LOG / "pass-1.2.7-moa-synth-input.txt").write_text(
        "A\n" + (ca["code"].get("text") or "") + "\nB\n" + (cb["code"].get("text") or "")
    )
    ...
    (LOG / "pass-1.2.7-moa-synth.txt").write_text(synth.get("text") or "")
    print(
        json.dumps(
            {
