#!/usr/bin/env python3
"""Single-worker qwen3.6-35b-ud titan. Not MoA. 3600s/request."""
import json, time, urllib.request
from pathlib import Path

LOG = Path("/Users/davidpence/kingdom-rush-clone/sprint-logs")
TASK = (LOG / "pass-1.3.8-qwen35-prompt.md").read_text()
...
CODE_SYS = (
    ...
    "and keep make(\"enemy_titan\", 88, 80, (ctx) => drawTitan(ctx, 0)); "
...
def main():
    ...
    (LOG / "pass-1.3.8-qwen35.json").write_text(json.dumps(rec, indent=2)[:200000])
    (LOG / "pass-1.3.8-qwen35-code.txt").write_text(code.get("text") or "")
    print(
        json.dumps(
            {
                "ok": code.get("ok"),
