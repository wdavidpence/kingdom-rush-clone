#!/usr/bin/env python3
"""Single-worker qwen3.6-35b-ud captain. Not MoA. 3600s/request."""
import json, time, urllib.request
from pathlib import Path

LOG = Path("/Users/davidpence/kingdom-rush-clone/sprint-logs")
TASK = (LOG / "pass-1.3.14-qwen35-prompt.md").read_text()
...
CODE_SYS = (
    "Output unified diffs only. Target live const drawHeroCaptainIdle = (ctx) => { "
    "and keep make(\"hero_captain_idle\", 64, 72, drawHeroCaptainIdle); "
    "Do not change 64, 72. Do not edit drawScout, drawBrute, drawFlyer, drawTitan, "
    "drawBossIdle, projectile_magic, drawHeroCaptainAttack, or drawHeroCaptainAbility. "
...
def main():
    ...
    (LOG / "pass-1.3.14-qwen35.json").write_text(json.dumps(rec, indent=2)[:200000])
    (LOG / "pass-1.3.14-qwen35-code.txt").write_text(code.get("text") or "")
    print(
        json.dumps(
            {
                "ok": code.get("ok"),
