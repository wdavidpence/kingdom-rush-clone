KRC 1.3.2 MoA ONE task: rewrite drawScout so the scout is a readable character at play scale.

Play scale is 40x36px (make 80x72, game.js setScale(base.size/30) with size 15).
Wow bar: at 40px you must read HEAD + EARS + SPEAR + BODY without squinting. Fat #141008 outline (line 2.8-3.2). Huge cream eyes. Lime tunic vs dark outline. No hairline folds, no 0.8px rims that vanish.

Keep make("enemy_scout", 80, 72 and all enemy_scout_w0..w3 / dead wrappers calling drawScout / drawScoutDead.
Do NOT change canvas 80,72. Do NOT rename drawScout. Keep face(ctx, cx, cy, eye, pupil, angry) if you draw a face.

LIVE make wrappers (do not change signatures):
...
LIVE drawScout (replace the function body; keep the name and (ctx, frame = 0) arity):
...
Allowed only: size: 15 → size: 18. Nothing else in that file.

HARD: do not touch { x: 100, y: 375 }, bannerY=98, campaign, version, other enemies, towers.
...
```
Hunks must apply to LIVE drawScout and the LIVE make("enemy_scout", 80, 72 wrappers.
