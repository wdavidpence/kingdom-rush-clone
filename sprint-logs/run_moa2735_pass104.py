#!/usr/bin/env python3
"""moa2735 pass 1.0.104: 24-28px barracks doorway guards. 1800s each."""
import json, time, urllib.request
from pathlib import Path

LOG = Path("/Users/davidpence/kingdom-rush-clone/sprint-logs")
PROMPT = (LOG / "pass-1.0.104-moa-prompt.md").read_text()
MODELS = [
...
def main():
    for name, url, model in MODELS:
        ...
        (LOG / f"pass-1.0.104-{name}.json").write_text(json.dumps(slim, indent=2))
        (LOG / f"pass-1.0.104-{name}.txt").write_text(rec.get("content") or rec.get("error") or "")
