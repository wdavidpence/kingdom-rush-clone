#!/usr/bin/env python3
"""Single-worker qwen3.6-35b-ud warden. Not MoA. 3600s/request."""
import json, time, urllib.request
from pathlib import Path

LOG = Path("/Users/davidpence/kingdom-rush-clone/sprint-logs")
TASK = (LOG / "pass-1.3.11-qwen35-prompt.md").read_text()
...
CODE_SYS = (
    "Output unified diffs only. Target live const drawBossIdle = (ctx, frame = 0) => { "
    "and keep make(\"enemy_boss\", 96, 88, (ctx) => drawBossIdle(ctx, 0)); "
    "Do not change 96, 88. Do not edit drawScout, drawBrute, or drawFlyer. "
    ...
    "Optional game-data.js: only boss size 30 to 32. "
...
def main():
    ...
    (LOG / "pass-1.3.11-qwen35.json").write_text(json.dumps(rec, indent=2)[:200000])
    (LOG / "pass-1.3.11-qwen35-code.txt").write_text(code.get("text") or "")
    print(
        json.dumps(
            {
                "ok": code.get("ok"),
