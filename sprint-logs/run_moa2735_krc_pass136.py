#!/usr/bin/env python3
"""PenceMOA qwen2735 1.3.4 locked flyer identity. Parallel, 3600s/request."""
import json, time, urllib.request, concurrent.futures
from pathlib import Path

LOG = Path("/Users/davidpence/kingdom-rush-clone/sprint-logs")
TASK = (LOG / "pass-1.3.4-moa-prompt.md").read_text()
...
CODE_SYS = (
    "Output unified diffs only. Target live const drawFlyer = (ctx, frame = 0) => { "
    "and keep make(\"enemy_flyer\", 80, 72, (ctx) => drawFlyer(ctx, 0)); "
    "Do not change 80, 72. Do not edit drawScout or drawBrute. "
    ...
    "Optional game-data.js: only flyer size 15 to 18. "
    "Do not touch { x: 100, y: 375 } or bannerY=98."
)
SYNTH_SYS = (
    "Synthesize ONE final set of diffs that apply to live drawFlyer and make(\"enemy_flyer\", 80, 72. "
    "Keep 80, 72. Do not touch drawScout or drawBrute. No fake APIs. No CONFIG. Optional: flyer size 15 to 18 only."
)
...
def main():
    ...
    (LOG / "pass-1.3.4-moa-a.json").write_text(json.dumps(ca, indent=2)[:200000])
    (LOG / "pass-1.3.4-moa-b.json").write_text(json.dumps(cb, indent=2)[:200000])
    (LOG / "pass-1.3.4-moa-synth-input.txt").write_text(
        "A\n" + (ca["code"].get("text") or "") + "\nB\n" + (cb["code"].get("text") or "")
    )
    ...
    (LOG / "pass-1.3.4-moa-synth.txt").write_text(synth.get("text") or "")
    print(
        json.dumps(
            {
