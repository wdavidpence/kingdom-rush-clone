...
CODE_SYS = (
    "Output unified diffs only. Target live make(\"projectile_magic\", 36, 36, (ctx) => { "
    "Do not change 36, 36. Do not edit drawScout, drawBrute, drawFlyer, drawTitan, or drawBossIdle. "
    "Use only poly/linGrad/radGrad/ellipse/rounded/shadow/speckles "
    "with live arities: shadow(ctx,x,y,rx,ry,a) speckles(ctx,x,y,w,h,n,color,size). "
    "Never invent CONFIG, settings, projectileScale, make(fn), make(name, fn), or renderSetPiece. "
    "Optional game.js: only the live setScale line, mage 0.9 to 1.2. "
    "Do not touch { x: 100, y: 375 } or bannerY=98."
)
SYNTH_SYS = (
    "Synthesize ONE final set of diffs that apply to live make(\"projectile_magic\", 36, 36. "
    "Keep 36, 36. Do not touch drawScout, drawBrute, drawFlyer, drawTitan, or drawBossIdle. "
    "No fake APIs. No CONFIG. Optional: only the live setScale mage 0.9 to 1.2."
)
...
def main():
    ...
    (LOG / "pass-1.3.13-moa-a.json").write_text(json.dumps(ca, indent=2)[:200000])
    (LOG / "pass-1.3.13-moa-b.json").write_text(json.dumps(cb, indent=2)[:200000])
    (LOG / "pass-1.3.13-moa-synth-input.txt").write_text(
        "A\n" + (ca["code"].get("text") or "") + "\nB\n" + (cb["code"].get("text") or "")
    )
    ...
    (LOG / "pass-1.3.13-moa-synth.txt").write_text(synth.get("text") or "")
    print(
        json.dumps(
            {
