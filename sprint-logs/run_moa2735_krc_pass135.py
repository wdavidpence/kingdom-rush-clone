#!/usr/bin/env python3
"""PenceMOA qwen2735 1.3.3 locked brute identity. Parallel, 3600s/request."""
import json, time, urllib.request, concurrent.futures
from pathlib import Path

LOG = Path("/Users/davidpence/kingdom-rush-clone/sprint-logs")
TASK = (LOG / "pass-1.3.3-moa-prompt.md").read_text()
...
CODE_SYS = (
    "Output unified diffs only. Target live const drawBrute = (ctx, frame = 0) => { "
    "and keep make(\"enemy_brute\", 80, 72, (ctx) => drawBrute(ctx, 0)); "
    "Do not change 80, 72. Do not edit drawScout. "
    ...
    "Optional game-data.js: only brute size 18 to 20. "
    "Do not touch { x: 100, y: 375 } or bannerY=98."
)
SYNTH_SYS = (
    "Synthesize ONE final set of diffs that apply to live drawBrute and make(\"enemy_brute\", 80, 72. "
    "Keep 80, 72. Do not touch drawScout. No fake APIs. No CONFIG. Optional: brute size 18 to 20 only."
)
...
def main():
    ...
    (LOG / "pass-1.3.3-moa-a.json").write_text(json.dumps(ca, indent=2)[:200000])
    (LOG / "pass-1.3.3-moa-b.json").write_text(json.dumps(cb, indent=2)[:200000])
    (LOG / "pass-1.3.3-moa-synth-input.txt").write_text(
        "A\n" + (ca["code"].get("text") or "") + "\nB\n" + (cb["code"].get("text") or "")
    )
    ...
    (LOG / "pass-1.3.3-moa-synth.txt").write_text(synth.get("text") or "")
    print(
        json.dumps(
            {
